NORMAL_LIST = [
    "凌阳",
    "安可",
    "卡卡罗",
    "鉴心",
    "维里奈",
    "漂泊者",
    "漂泊者·衍射",
    "漂泊者·湮灭",
    "漂泊者·气动",
    "漂泊者·热熔",
    "漂泊者·冷凝",
    "漂泊者·导电",
    "千古洑流",
    "停驻之烟",
    "擎渊怒涛",
    "漪澜浮录",
    "浩境粼光",
    "镭射切变",
    "源能机锋",
    "相位涟漪",
    "脉冲协臂",
    "玻色星仪",
]

NORMAL_LIST_IDS = [1104, 1203, 1301, 1405, 1503]

card_sort_map = {
    "生命": "0",
    "攻击": "0",
    "防御": "0",
    "谐度破坏增幅": "0",
    "共鸣效率": "0%",
    "偏谐值累积效率": "0.0%",
    "暴击": "0.0%",
    "暴击伤害": "0.0%",
    "属性伤害加成": "0.0%",
    "治疗效果加成": "0.0%",
    "普攻伤害加成": "0.0%",
    "重击伤害加成": "0.0%",
    "共鸣技能伤害加成": "0.0%",
    "共鸣解放伤害加成": "0.0%",
}

ATTRIBUTE_ID_MAP = {1: "冷凝", 2: "热熔", 3: "导电", 4: "气动", 5: "衍射", 6: "湮灭"}
ATTRIBUTE_NAME_SET = set(ATTRIBUTE_ID_MAP.values())
WEAPON_TYPE_ID_MAP = {1: "长刃", 2: "迅刀", 3: "佩枪", 4: "臂铠", 5: "音感仪"}
DEAFAULT_WEAPON_ID = {1: 21010011, 2: 21020011, 3: 21030011, 4: 21040011, 5: 21050011}
SKILL_MAP = {
    "常态攻击": "1",
    "共鸣技能": "2",
    "共鸣回路": "7",
    "共鸣解放": "3",
    "变奏技能": "6",
    "延奏技能": "8",
    "谐度破坏": "17",
}

SPECIAL_CHAR = {
    "1501": ["1501", "1502"],  # 光主
    "1502": ["1501", "1502"],
    "1604": ["1604", "1605"],  # 暗主
    "1605": ["1604", "1605"],
    "1406": ["1406", "1408"],  # 风主
    "1408": ["1406", "1408"],
    "1309": ["1309", "1310"],  # 雷主
    "1310": ["1309", "1310"],
}

SPECIAL_CHAR_INT = {
    1501: [1501, 1502],  # 光主
    1502: [1501, 1502],
    1604: [1604, 1605],  # 暗主
    1605: [1604, 1605],
    1406: [1406, 1408],  # 风主
    1408: [1406, 1408],
    1309: [1309, 1310],  # 雷主
    1310: [1309, 1310],
}

SPECIAL_CHAR_INT_ALL = [1501, 1502, 1604, 1605, 1406, 1408, 1309, 1310]

def randomize_special_char_id(char_id: int) -> int:
    """排行展示时随机将特殊角色替换为配对形态，保护用户隐私/公平性"""
    import random
    if char_id in SPECIAL_CHAR_INT:
        return random.choice(SPECIAL_CHAR_INT[char_id])
    return char_id


SPECIAL_CHAR_NAME = {
    "1501": "光主",
    "1502": "光主",
    "1604": "暗主",
    "1605": "暗主",
    "1406": "风主",
    "1408": "风主",
    "1309": "雷主",
    "1310": "雷主",
}
SPECIAL_CHAR_ID = {
    "漂泊者·衍射·男": "1501",
    "漂泊者·衍射·女": "1502",
    "漂泊者·湮灭·男": "1605",
    "漂泊者·湮灭·女": "1604",
    "漂泊者·气动·男": "1406",
    "漂泊者·气动·女": "1408",
    "漂泊者·导电·男": "1309",
    "漂泊者·导电·女": "1310",
}
ID_FULL_CHAR_NAME = {
    "1501": "漂泊者·衍射",
    "1502": "漂泊者·衍射",
    "1604": "漂泊者·湮灭",
    "1605": "漂泊者·湮灭",
    "1406": "漂泊者·气动",
    "1408": "漂泊者·气动",
    "1309": "漂泊者·导电",
    "1310": "漂泊者·导电",
}

NAME_ALIAS = {"漂泊者·湮灭": "暗主", "漂泊者·衍射": "光主", "漂泊者·气动": "风主", "漂泊者·导电": "雷主", "秧秧·玄翎": "玄翎"}

SPECIAL_CHAR_RANK_MAP = {
    "1501": "1501",
    "1502": "1501",
    "1604": "1604",
    "1605": "1604",
    "1406": "1406",
    "1408": "1406",
    "1309": "1309",
    "1310": "1309",
}


SONATA_FIRST_ID = {
    "凝夜白霜": [6000044],
    "熔山裂谷": [6000091, 390080007],
    "彻空冥雷": [390080003, 6000039, 6000088, 6000089],
    "啸谷长风": [6000086, 6000043],
    "浮星祛暗": [6000059, 6000045],
    "沉日劫明": [6000090, 6000053, 6000042],
    "隐世回光": [6000060, 390080005],
    "轻云出月": [6000052, 390080005],
    "不绝余音": [6000048],
    "凌冽决断之心": [6000083],
    "高天共奏之曲": [6000085, 6000089],
    "幽夜隐匿之帷": [6000082, 6000087],
    "此间永驻之光": [6000092],
    "无惧浪涛之勇": [6000084],
    "流云逝尽之空": [6000106, 6000113],
    "愿戴荣光之旅": [6000106, 6000113],
    "奔狼燎原之焰": [6000114],
    "失序彼岸之梦": [6000115],
    "荣斗铸锋之冠": [6000121, 6000160],
    "息界同调之律": [6000116],
    "焚羽猎魔之影": [6000167],
    "命理崩毁之弦": [6000167],
    "逆光跃彩之约": [6000189],
    "星构寻辉之环": [6000190],
    "流金溯真之式": [6000179],
    "长路启航之星": [6000191],
    "听唤语义之愿": [6000192],
    "剪心辑梦之影": [6000198],
    "雪落无声之愿": [6000199, 6000195],
    "斑驳粉饰之沫": [6000200],
    "碎梦亡鬼之魇": [6000201],
    "羽落空尘之歌": [6000218, 6000216],
    "清邪荡煞之心": [6000217],
    "冥途夜行之灯": [6000217],
}

# 换合鸣强替首位时按角色取默认 (已装合法首位则不替换); 未命中角色则用 SONATA_FIRST_ID[套装][0]
SONATA_FIRST_ID_BY_ROLE = {
    "雪落无声之愿": {1109: 6000195, 1108: 6000199},  # 洛瑟菈-格洛犸图, 绯雪-虚造神型
    "羽落空尘之歌": {1610: 6000218, 1110: 6000216},  # 玄翎-千傀重楼, 穗穗-封庭械囿
    "荣斗铸锋之冠": {1306: 6000121, 1410: 6000160},  # 奥古斯塔-伪作的神王, 尤诺-海之女
}


SHORT_NAME = {
    6000106: "芙露德莉斯",
    6000167: "利维亚坦",
    6000199: "虚造神型",
    6000200: "达妮娅·骗术师",
    6000201: "梦魇亚当·重锤",
}


def get_short_name(sonata_id: int, sonata_name: str) -> str:
    return SHORT_NAME.get(sonata_id, sonata_name)
